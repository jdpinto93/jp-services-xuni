...
class Tipo
{
    ...
    {
        if (is_admin()) {
            $this->data = file_get_contents(JP_SERVICES_PATH . '/Parts/Assets/config/fields-datos-tipo.json');
            $this->meta_fields = json_decode($this->data, true);

            add_action('tipo_add_form_fields', array($this, 'create_fields'), 10, 2);
            add_action('tipo_edit_form_fields', array($this, 'edit_fields'),  10, 2);
            add_action('created_tipo', array($this, 'save_fields'), 10, 1);
            add_action('edited_tipo',  array($this, 'save_fields'), 10, 1);
...
if (class_exists('Parts\Metaboxes\Tipo')) {
    new Tipo();
};